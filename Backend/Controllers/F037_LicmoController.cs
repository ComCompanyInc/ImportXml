...
namespace BackendApp.Controllers
{
    ...
    public class F037_LicmoController : ControllerBase
    {
        private readonly F037_LicmoService _f037_licmoService;

        public F037_LicmoController(F037_LicmoService f037_licmoService)
        {
            _f037_licmoService = f037_licmoService;
        }
        ...
        public async Task<List<ErrorResponseDto>> ImportXmlData()
        {
            ...
            XmlSerializer serializer = new XmlSerializer(typeof(DocumentDto<F37DataDto>)); // приводим данные потока к классу-сериализатору xml в обьект XmlSerializer
            DocumentDto<F37DataDto> dataContainer = (DocumentDto<F37DataDto>)serializer.Deserialize(stringReader); // при помощи класса XmlSerializer - сериализуем наш обьект в DocumentDto<F31DataDto>
            stringReader.Close(); // ОБЯЗАТЕЛЬНО закрываем

            //return dataContainer;

            // 3. Сохраняем данные
            return await _f037_licmoService.SaveDataFromF37(dataContainer);
        }
    }
}
