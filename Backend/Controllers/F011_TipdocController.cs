...
namespace BackendApp.Controllers
{
    ...
    public class F011_TipdocController : ControllerBase
    {
        private readonly F011_TipdocService _f011_TipdocService;

        public F011_TipdocController(F011_TipdocService f011_TipdocService)
        {
            _f011_TipdocService = f011_TipdocService;
        }
        ...
        public async Task<bool /*DocumentDto<F31DataDto>*/> ImportXmlData(/*[FromBody] DocumentDto<F31DataDto> dataContainer*/)
        {
            ...
            XmlSerializer serializer = new XmlSerializer(typeof(DocumentDto<F11DataDto>)); // приводим данные потока к классу-сериализатору xml в обьект XmlSerializer
            DocumentDto<F11DataDto> dataContainer = (DocumentDto<F11DataDto>)serializer.Deserialize(stringReader); // при помощи класса XmlSerializer - сериализуем наш обьект в DocumentDto<F31DataDto>
            stringReader.Close(); // ОБЯЗАТЕЛЬНО закрываем

            //return dataContainer;

            // 3. Сохраняем данные
            return await _f011_TipdocService.SaveDataFromF11(dataContainer);
        }
    }
}
