...
namespace BackendApp.Controllers
{
    ...
    public class F006_VidExpController : ControllerBase
    {
        private readonly F006_VidExpService _f006_VidExpService;

        public F006_VidExpController(F006_VidExpService f006_VidExpService)
        {
        ...
        public async Task<bool /*DocumentDto<F31DataDto>*/> ImportXmlData(/*[FromBody] DocumentDto<F31DataDto> dataContainer*/)
        {
            ...
            XmlSerializer serializer = new XmlSerializer(typeof(DocumentDto<F6DataDto>)); // приводим данные потока к классу-сериализатору xml в обьект XmlSerializer
            DocumentDto<F6DataDto> dataContainer = (DocumentDto<F6DataDto>)serializer.Deserialize(stringReader); // при помощи класса XmlSerializer - сериализуем наш обьект в DocumentDto<F31DataDto>
            stringReader.Close(); // ОБЯЗАТЕЛЬНО закрываем

            // 3. Сохраняем данные
            return await _f006_VidExpService.SaveDataFromF6(dataContainer);
        }
    }
}
