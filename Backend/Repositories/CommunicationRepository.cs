    ...
    public class CommunicationRepository : AbstractBaseRepository<Communication>, ISearchData<Communication>
    {
        ...
        public async Task<List<Communication>> GetDataBySearchFilter(Communication FilterDto)
        {
            ...
            if (!FilterDto.Phone.IsNullOrEmpty())
            {
                communication = communication
                    .Where(c =>
                        c.Phone.Contains(FilterDto.Phone)
                    );
            }

            if (!FilterDto.Fax.IsNullOrEmpty())
            {
                communication = communication
                    .Where(c =>
                        c.Fax.Contains(FilterDto.Fax)
                    );
            }

            if (!FilterDto.Email.IsNullOrEmpty())
            {
                communication = communication
                    .Where(c =>
                        c.Email.Contains(FilterDto.Email)
                    );
            }

            if (!FilterDto.HotLine.IsNullOrEmpty())
            {
                communication = communication
                    .Where(c =>
                        c.HotLine.Contains(FilterDto.HotLine)
                    );
            }

            if (!FilterDto.Site.IsNullOrEmpty())
            {
                communication = communication
                    .Where(c =>
