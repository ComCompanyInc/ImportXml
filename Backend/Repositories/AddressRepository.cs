    ...
    public class AddressRepository : AbstractBaseRepository<Address>, ISearchData<Address>
    {
        ...
        public async Task<List<object>> GetDataBySearchFilter(Dictionary<string, object> filter)
        {
            ...
            /*if (!FilterDto.Name.IsNullOrEmpty())
            {
                addresses = addresses
                    .Where(c =>
                        c.Name.Contains(FilterDto.Name)
                    );
            }

            if (!FilterDto.Index.IsNullOrEmpty())
            {
                addresses = addresses
                    .Where(c =>
                        c.Index.Contains(FilterDto.Index)
                    );
            }

            if (!FilterDto.Oktmo.IsNullOrEmpty())
            {
                addresses = addresses
                    .Where(c =>
                        c.Oktmo.Contains(FilterDto.Oktmo)
                    );
            }

            if (!FilterDto.AddressCode.IsNullOrEmpty())
            {
                addresses = addresses
                    .Where(c =>
